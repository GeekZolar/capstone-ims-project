import { NavLink, useNavigate } from 'react-router-dom'
import { LogOut } from 'lucide-react'
import { navigation } from '../../config/navigation'
import clsx from 'clsx'
import { useAuthStore } from '../../store/authStore'
import type { UserRole } from '../../types/ims'

const roleWeight: Record<UserRole, number> = {
  system_admin: 5,
  po_approver: 4,
  po_creator: 3,
  forecast_editor: 3,
  inventory_manager: 2,
  read_only: 1,
}

export const AppSidebar = ({ collapsed = false }: { collapsed?: boolean }) => {
  const navigate = useNavigate()
  const logout = useAuthStore((state) => state.logout)
  const user = useAuthStore((state) => state.user)

  const handleLogout = () => {
    logout()
    navigate('/login')
  }

  return (
    <aside
      className={clsx(
        'hidden h-screen flex-col border-r border-[rgb(var(--border))] bg-[rgb(var(--card))] py-6 md:fixed md:inset-y-0 md:left-0 md:flex',
        collapsed ? 'w-16 px-2' : 'w-64 px-4',
      )}
    >
      <div className={clsx('mb-8 flex flex-col items-center gap-3', collapsed && 'mb-4')}>
        <img
          src="/assets/img/S&R_logo.png"
          alt="S&R Foods logo"
          className={clsx(
            'rounded-xl bg-white object-contain',
            collapsed ? 'h-10 w-10' : 'h-20 w-20',
          )}
        />
        {!collapsed && (
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
              S&R Foods <span className="text-slate-900 font-bold">IMS</span>
            </p>
          </div>
        )}
      </div>
      <nav className="flex flex-1 flex-col gap-1">
        {navigation
          .filter((item) => {
            if (!item.minRole) return true
            if (!user) return false
            return roleWeight[user.roleAlt] >= roleWeight[item.minRole]
          })
          .map((item) => {
          const Icon = item.icon
          return (
            <NavLink
              key={item.path}
              to={item.path}
              title={collapsed ? item.label : undefined}
              className={({ isActive }) =>
                clsx(
                  'flex items-center rounded-lg py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-100',
                  collapsed ? 'justify-center px-2' : 'gap-3 px-3',
                  isActive && 'bg-slate-100 text-slate-900',
                )
              }
            >
              <Icon className="h-4 w-4" />
              {!collapsed && item.label}
            </NavLink>
          )
        })}
      </nav>
      <button
        onClick={handleLogout}
        title={collapsed ? 'Logout' : undefined}
        className={clsx(
          'mt-4 flex cursor-pointer items-center rounded-lg py-2 text-sm font-medium text-rose-600 transition hover:bg-rose-50',
          collapsed ? 'justify-center px-2' : 'gap-3 px-3',
        )}
        aria-label="Log out"
      >
        <LogOut className="h-4 w-4" />
        {!collapsed && 'Logout'}
      </button>
      {!collapsed && (
        <div className="rounded-xl bg-[rgb(var(--bg-muted))] p-4 text-xs text-[rgb(var(--muted))]">
          <p className="font-semibold text-[rgb(var(--text))]">Sync status</p>
          <p className="mt-2">QuickBooks: Healthy</p>
          <p>WMS: Healthy</p>
          <p>Sales Channels: Degraded</p>
        </div>
      )}
    </aside>
  )
}
